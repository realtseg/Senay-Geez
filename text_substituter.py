...
class TextSubstituter:
    def __init__(self):
        self.substitutions = {}
        self.buffer = deque(maxlen=20)  # Increased buffer size
        self.lock = Lock()
        self.enabled = True
        self.config_file = "config.csv"
        self.last_modified = 0
        self.typing_delay = 0.05  # Delay to allow multi-character sequences
    ...
    def check_substitution(self):
        """Check if buffer ends with any substitution key - only check after delay"""
        with self.lock:
            if not self.buffer:
                return None, None
            ...
            for key, value in sorted_subs:
                if buffer_str.endswith(key):
                    # Only substitute if we have the exact sequence
                    return key, value
            return None, None
    ...
    def start_monitoring(self):
        ...
        print("Note: There's a small delay to allow multi-character sequences")
        
        # Register hotkeys
        keyboard.add_hotkey('f12', self.toggle_enabled)
    ...
    def toggle_enabled(self):
        """Toggle substitution on/off"""
        self.enabled = not self.enabled
        status = "ENABLED" if self.enabled else "DISABLED"
        print(f"Text substitution {status}")
...
def main():
    ...
    try:
        ...
        if not ctypes.windll.shell32.IsUserAnAdmin():
            print("Warning: For best compatibility, run as Administrator")
            print("This helps the application work in all programs.")
            print()
    except:
        pass
